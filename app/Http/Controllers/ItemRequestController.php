...
class ItemRequestController extends Controller
{
    ...
    {
        $barangs = GoodItem::where('stok', '>', 0)
            ->orderBy('name')
            ->get();

        $pengajuans = ItemRequest::with('goodItem')
            ->where('user_id', Auth::id())
            ->whereIn('status', ['menunggu', 'ditolak'])
            ->latest()
            ->get();

        return view('item_request', compact('barangs', 'pengajuans'));
    }
    ...
    public function tambahKeranjang(Request $request)
    {
        ...
        $barang = GoodItem::findOrFail($request->barang_id);
    ...
    public function ajukan()
    {
        ...
        foreach ($keranjang as $item) {
            $barang = GoodItem::find($item['barang_id']);
            if (!$barang || $barang->stok < $item['jumlah']) continue;

            ItemRequest::create([
                'user_id'   => Auth::id(),
                'good_item_id' => $item['barang_id'],
                'jumlah'    => $item['jumlah'],
    ...
    {
        $peminjaman = ItemRequest::where('user_id', Auth::id())
